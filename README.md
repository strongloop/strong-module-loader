...
We can load up all the dogs like so (app.js):
    ...
    var moduleLoader = require('strong-module-loader').create('my-app');
...
A module class is a `node_module` that exports a constructor inheriting from the `Module` class.
    ...
    var Module = require('strong-module-loader').Module;
...
After your program runs `require('strong-module-loader')` the `require()` function's behavior will change slightly to make referencing module instances simpler. Since some module instances may not have any program specific code, they can't be `require()`d with `node`'s existing require() implementation.
    
## Config Loader

`strong-module-loader` inherits from [strong-config-loader](https://github.com/strongloop/sl-config-loader).
...
You may also provide aliases to any module path when creating a `ModuleLoader`.
    ...
    var moduleLoader = require('strong-module-loader').create('my-app', {alias: {'foo': 'myBundle/node_modules/foo'}});
