...
PatchedModule.loadModuleFromPath = function (path, root) {
  // resolve paths relative to the root (eg. NODE_PATH, etc)
  var paths = PatchedModule.resolveLookupPaths(root)[1];

  // add in `node_module` paths
  paths = PatchedModule.nodeModulePaths(root).concat(paths);
  
  // add in `extraPaths`
  paths = PatchedModule.extraPaths.concat(paths);
  
  // load the actual module
  return PatchedModule.load(PatchedModule.resolveFilename(path, {paths: paths}));
}

/**
 * Alias functions in case they need to be patched in the future.
 */
