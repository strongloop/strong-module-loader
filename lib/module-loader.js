...
var ConfigLoader = require('asteroid-config-loader')
  ...
  , fs = require('fs')
  , path = require('path')
  , debug = require('debug')('asteroid-module-loader')
  , util = require('util')
...
ModuleLoader.prototype.loadType = function (name) {
  ...
  var Type = PatchedModule.load(PatchedModule.resolveFilename(alias, {paths: [this.root]}));
...
ModuleLoader.prototype.getTypeFromConfig = function (config, configPath) {
  ...
  if(typeName[0] === '.') {
    try {
      Type = require(path.join(path.resolve(this.root), path.dirname(configPath), typeName));
    } catch(e) {
      fail(e);
    }
  } else {
